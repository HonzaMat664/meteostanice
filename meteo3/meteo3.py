...
import smbus
import struct
import subprocess  # pro rsync

# ==========================
# KONSTANTY
# ==========================
CSV_FILE = "/home/honza/nas-web/data3.csv"  # nové jméno souboru
...
# ==========================
# MASTER RPi – nastavení pro odesílání
# ==========================
MASTER_USER = "honza"
MASTER_HOST = "192.168.1.159"
MASTER_PATH = "/home/honza/nas-web/"
...
def write_csv(row):
    ...
    with open(CSV_FILE, 'a', newline='') as f:
        ...
        writer.writerow(row)
    # po zápisu odeslat na master
    send_to_master(CSV_FILE)

# ==========================
# Funkce pro odeslání na master RPi
# ==========================
def send_to_master(local_file):
    try:
        subprocess.run([
            "rsync", "-az", local_file,
            f"{MASTER_USER}@{MASTER_HOST}:{MASTER_PATH}"
        ], check=True)
        print(f"Soubor {local_file} odeslán na {MASTER_HOST}")
    except subprocess.CalledProcessError as e:
        print(f"Chyba při odesílání souboru: {e}")
